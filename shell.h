...
int printEXDir(void) {
  ...
  if (getcwd(cwd, sizeof(cwd)) != NULL) {
    printf(ANSI_COLOR_RED "mark@" ANSI_COLOR_RESET "EL_ABBAS-" ANSI_COLOR_GREEN
                          "%s" ANSI_COLOR_RED ":~$",
           cwd);
  } else {
    perror("getcwd() error");
...
int parseSpace(char* str, char** parsed) {
  ...
  for (index = 0; index < MAXLIST; index++) {
    ...
    if (parsed[index] == NULL) break;
    if (strlen(parsed[index]) == 0) index--;
  }
  return index;
}
...
void exec_Args(char** parsed, int size) {
  // Forking a child
  pid_t pid = fork();
  ...
  if (pid == -1) {
    ...
    if (size > 0 && !strcmp(parsed[size - 1], "&")) {
      printf("I'm here\n");
      exit(0);
    } else {
      execvp(parsed[0], parsed);
...
// process on line
int process_Line(char* str, char** parsed, char** parsedpipe, int* size) {
  char* strpiped[3];
  int piped = 0;
